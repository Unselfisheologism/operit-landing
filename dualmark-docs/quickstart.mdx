...
Dualmark works in any framework that speaks `Request` / `Response`. First-class adapters ship for Astro, Next.js, and Cloudflare Workers; everything else uses `@dualmark/core` directly.

Pick your stack below — every tab is a complete, self-contained install.

<Tabs items={["Astro", "Next.js", "Cloudflare Workers", "Any framework"]}>
...
For SvelteKit, Remix, Nuxt, Hono, Express, raw Node, Bun, Deno — anything with the Fetch API — use `@dualmark/core` directly.
